...
def _conv2d_relu(vgg_layers, prev_layer, layer, layer_name):
    ...
    """
    W_a, b_a = _weights(vgg_layers, layer, layer_name)
    W = tf.constant(W_a)
    b = tf.constant(b_a)
    output_layer = tf.nn.conv2d(prev_layer, W, strides=[1,1,1,1], padding="SAME")
    output_layer_bias = tf.nn.bias_add(output_layer, b)
    relu = tf.nn.relu(output_layer_bias)
    return relu
...
def _avgpool(prev_layer):
    ...
    """
    avgpool = tf.nn.avg_pool(prev_layer, ksize=[1,2,2,1], strides=[1,1,1,1], padding='SAME')
    return avgpool
